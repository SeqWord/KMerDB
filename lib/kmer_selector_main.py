...
class kmer_selector:
    def __init__(self, input_path: str, out_path: str, tmp_path: str, project: str, 
            min_k: int = 0, max_k: int = 0, flg_use_NN: bool = True, mmcs: int = 30,    # mmcs - Minimal Median Cluster Size to use Neural Network (NN)
    ...
    def execute(self, 
            label_taxon: str = "species",               # '' | species | genus
            ...
            flg_matrix_per_k: bool = True,
            flg_print_classifier: bool = False, 
            flg_step_by_step: bool = False,
        ):
        ...
        self.process_leaves(leaves=leaves, 
                tmp_root=tmp_root,
                label_taxon=label_taxon,            # '' | species | genus
                data_type=data_type,                # digit | count | z-score
                digit_map=digit_map,                # four member list, aplicable for datatype 'digit' to transform '000', '001', '011', '111' to numbers
                matrix_geometry=matrix_geometry,    # whole | upper | lower
                flg_matrix_per_k=flg_matrix_per_k
            )
    ...
    def process_leaves(self, 
            leaves: list, 
            tmp_root: str, 
            label_taxon: str = "species", 
            data_type: str = "digit", 
            digit_map: list = [-2, -1, 1, 2],
            matrix_geometry: str = "lower",
            flg_matrix_per_k=True
        ):
    ...
    def _retrieve_value(self, acc_list: List[str], marker_list: List[str]) -> pd.DataFrame:
        ...
        for fname, acc_sublist in file_groups.items():
            ...
            try:
                values = wdb_reader.get_values(fname, acc_sublist, marker_list)
            except Exception:
                # Retrieval failure → keep "0"
                continue
