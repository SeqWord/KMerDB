...
def execute(path: str, min_k: int = 4, max_k: int = 5,
    algorithm: str = "SPECTRAL",          # NJ | UPGMA | SPECTRAL
    output_file: str = "",
    output_format: str = "",
    ) -> tuple[tree, list[str]]:
    ...
    if max_k <= min_k:
        tools.msg(f"Max_k parameter {max_k} must be bigger than min_k {min_k}!")
        sys.exit()
    ...
    for fname in wdb_files:
        ...
        for g in data['genomes']:
            #g['file'] = os.path.basename(fname)[:-4]
            g['file'] = os.path.basename(fname)
        genomes += data['genomes']
        
        '''
        Genome record example:
        {
            'lineage': 'Python3|OligoDBViewer|v.2|input|chromosomes|Enterobacteriales|endosymbionts|Sodalis|NC_007712.1', 
            'accession': 'NC_007712.1', 
            'ID': 1, 
            'seqlength': 4171146, 
            'source': " 'E:\\\\MyPrograms\\\\Python3\\\\OligoDBViewer\\\\v.2\\\\input\\\\chromosomes\\\\Enterobacteriales\\\\endosymbionts\\\\Sodalis\\\\NC_007712.1.gbk'"
        }
        '''
        ...
        # Set total
        if not total:
            total = len(data['values'][0]) * 3
            
        # Each value is a long integer
        values += [int("1"+"".join(ls), 2) for ls in data['values']]
    ...
    for i in range(len(values) - 1):
        ...
        for j in range(i + 1, len(values)):
            ...
            matrix[-1].append((a ^ b).bit_count() / total)
    ...
    if algorithm.upper() == "UPGMA":
        pathways = upgma_paths_from_upper_triangle(matrix, labels, output_file=output_file)
    elif algorithm.upper() == "NJ":
        pathways = nj_paths_from_upper_triangle(matrix, labels, output_file=output_file)
    elif algorithm.upper() == "SPECTRAL":
        pathways = spa(matrix=matrix, 
            labels=labels, 
            max_cluster_number=7, 
            max_cluster_content=7,
            max_levels=5,
            force_k = 7,
...
def nj_paths_from_upper_triangle(
    ...
    while len(active) > 2:
        ...
        # Q-matrix: Q_ij = (m-2)*D_ij - r_i - r_j
        best = (math.inf, None, None)
