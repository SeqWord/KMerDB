...
###############################################################################
class Template:
    def __init__(self, title="", date="", version="1.0"):
    ...
    #### ACCESSIONS
    def add(self,wlength,x,y,count=1, matrix="lower", echo=False):  # matrix = "lover" | "upper" | "whole" / "entire"
        if (matrix == "lower" and int(x) > int(y)) or (matrix == "upper" and int(x) < int(y)):
            x, y = [y, x]
    ...
    def add_word(self,word,count=1):
        ...
        wlength,x,y,count = self.add(wlength,x,y,count)
...
class WordDB(Template):
    def __init__(self, title="", date="", version="1.0"):
        Template.__init__(self, title=title, date=date, version=version)
    ...
    def add_genome_ID(self, lineage, acc, ID=None):
        ...
        for wlength in self.db:
            for x in self.db[wlength]['x']:
                for y in self.db[wlength]['x'][x]:
                    self.insert_value(wlength,x,y,ID,'000')
        return ID
    ...
    """
    def add_genome(self, path, accession, lineage, wl1=8, wl2=14):
        # Read sequence file
        dataset = tools.openSeqFile(path)
        if not dataset:
            return
            
        seqlength = len(dataset['Sequence'])
    ...
    def process_sequence(self, lineage, sequence, ID, wl1=4, wl2=11):
        ...
        for wlength in range(wl1,wl2+1):
            mydb = Template()
            tmpWordDB = tools.count_words(sequence,wlength,mydb,True,"Count "+str(wlength)+"-mers: ")
            ...
            if infile and os.path.exists(infile):
                with open(infile) as f:
                    line = f.readline()
                    while line:
                        wlength,x,y = list(map(lambda v: int(v), line.split(",")))
                        if not tmpWordDB.has(wlength,x,y):
                            line = f.readline()
                            continue
                        count = tmpWordDB.get(wlength,x,y)
                        if self.db.has(wlength,x,y,True):
                            wlength,x,y,n = self.add(wlength,x,y,count)
                            value = tools.set_value(wlength, count, seqlength, True)
                            self.db.update_value(wlength,x,y,ID,value)
                        else:
                            line = f.readline()
                            continue
                        counter += 1
                ...
                for wlength,x,y in words:
                    if not tmpWordDB.has(wlength,x,y):
                        continue
                    count = tmpWordDB.get(wlength,x,y)
                    '''
                    #### TEMP
                    if wlength not in counts:
                        counts[wlength] = []
                    counts[wlength].append(count)
                    '''    
                    wlength,x,y,n = self.add(wlength,x,y,count,True)
                    self.append_value(wlength,x,y,ID,tools.set_value(wlength, count, seqlength, True))
                    counter += 1
                    if counter%99 == 0:
                        try:
                            bar(counter)
                ...
                raise ValueError(f"Inappropriate type of word collection: {type(words)}")
            del tmpWordDB
            del words
            bar.stop()
        ...
        #### TMP
        '''
        for wl in range(wl1, wl2 + 1, 1):
            counts[wl].sort()
            out_string += (
                f"wl={wl}, "
                f"{(10**6) * counts[wl][len(counts[wl])//4] / seqlength:.5f}, "
                f"{(10**6) * counts[wl][len(counts[wl])//2] / seqlength:.5f}, "
                f"{(10**6) * counts[wl][3 * len(counts[wl])//4] / seqlength:.5f}\n"
            )
            
        with open("word_counts.csv", "a") as f:
            f.write(out_string)
        '''    
        
    def next_ID(self):
        return len(self.genomes['acc'].items())
    
    # return the list of genome accessions sorted by their IDs in the database
    def get_genomes(self):
        accessions = list(self.genomes['acc'].keys())
        return accessions
    ...
    def delete_genome(self,acc):
        ...
        del self.genomes['acc'][acc]
        del self.genomes['genomes'][acc]
        for key in self.genomes['acc']:
            if self.genomes['acc'][key] > ID:
                self.genomes['acc'][key] -= 1
    ...
    def export_db(self, min_k=4, max_k=0, outfile=''):
        ...
        words = sorted(self.get_allWords(wl_lower=min_k, wl_upper=max_k, flg_add_words=True), 
            key = lambda w: [int(w[1]), int(w[3]), int(w[2])]) # sort by [wl, y, x] - lower matrix part
        data['words'] = [{
                        "word" : words[i][0],
