...
def openSeqFile(fname):
    if not os.path.exists(fname):
        return
    if fname[fname.rfind("."):] in (".fa",".fsa",".fas",".fst",".fna",".fasta"):
        return openFASTA(fname)
    elif fname[fname.rfind("."):] in (".gbk", ".gbf", ".gb"):
        return openGBK(fname)
...
def openFASTA(fname):
    ...
    for genome in genomes[1:]:
        ...
        if pos == -1:
            continue
        seqname = genome[:pos].strip()
        sequence = genome[pos:].replace('\n', '').upper()
...
def openGBK(fname):
    ...
    pos = strText.find("ACCESSION   ")
    if pos >= 0:
        end_pos = strText.find("\n", pos + 12)
        dataset['Accession'] = strText[pos + 12:end_pos].strip()

    pos = strText.find("  ORGANISM  ")
    if pos == -1:
        pos = strText.find("SOURCE      ")

    if pos >= 0:
        end_pos = strText.find("\n", pos + 12)
        organism = strText[pos + 12:end_pos].split()
        if organism:
            dataset['Genus'] = organism[0]
        if len(organism) > 1:
            dataset['Species'] = organism[1]
        if len(organism) > 2:
            dataset['Strain'] = " ".join(organism[2:])

    pos = strText.find('ORIGIN')
    if pos == -1:
        return dataset

    seq = strText[strText.find("\n", pos + 1) + 1:]
    seq = ''.join(filter(lambda x: x.isalpha(), seq)).upper()
    dataset['Sequence'] = seq

    return dataset
    
# Open FASTA file of a cluster of DNA reads
def openCluster(fname=None):
    if not fname:
        fname = askfilename([("Sequence files", "*.fas"),("Sequence files", "*.fst"),
            ("Sequence files", "*.fna"),("Sequence files", "*.fasta")])
    if not fname or not os.path.exists(fname):
        return
    try:
        seqlist = openFASTA(fname)
    except:
        return
    if not seqlist:
        return
    sequence = "N".join(seqlist.values())
    return sequence,fname
...
def dereplicate_and_concatenate(ls):
	if len(ls) < 2:
		return ls
	ls.sort()
	for i in range(len(ls)-1,0,-1):
...
def count_words(sequence,wlength,tmpWordDB,flg_progress=True,bar_text="Word count: "):
    words = sorted([
        sequence[i:i + wlength]
        for i in range(len(sequence) - wlength)
        if 'N' not in sequence[i:i + wlength].upper()
    ])
    word = words[-1]
    count = 1
    bar = None
